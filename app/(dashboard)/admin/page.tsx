...
export default async function AdminDashboard() {
  ...
  const critical = await prisma.grievance.count({
    where: { OR: [{ urgency: "Critical" }, { urgency: "High" }] },
  });
  const escalated = await prisma.grievance.count({
    ...
    <main className="flex min-h-[calc(100vh-6rem)] bg-background">
      ...
      <section className="flex-1 px-6 pb-10 lg:px-10">
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[1.35fr,1fr]">
            ...
            <div className="flex flex-wrap items-end justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  Monitor routing quality, response pace, and unresolved pain
                  points.
                </p>
              </div>
                ...
                >
                  <div>
                    <p className="mb-1 text-[11px] uppercase tracking-[0.18em] text-muted-foreground">
                      Support{" "}
                      {grievance.votes.filter((vote) => vote.value > 0).length -
                        grievance.votes.filter((vote) => vote.value < 0).length}{" "}
                      · {grievance.follows.length} following ·{" "}
                      {grievance.comments.length} comments
                    </p>
          ...
          <div className="clean-card p-6">
            ...
            <div className="mt-5 space-y-3">
              <Link
                href="/patterns"
                className="block rounded-2xl border border-border bg-background/40 p-4 transition-colors hover:border-foreground/20"
              >
                <p className="text-sm font-semibold text-foreground">
                  Pattern analysis
                </p>
                <p className="mt-1 text-xs text-muted-foreground">
                  Find recurring categories and department hotspots.
                </p>
              </Link>
              <Link
                href="/ai-settings"
              ...
              >
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  Manage the AI provider and model configuration.
                </p>
              </Link>
              <Link
                href="/users"
              ...
              >
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  Add admins/department users and control access.
                </p>
              </Link>
            </div>
          </div>
        ...
        <div className="clean-card p-6">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            High level view of trends and workload split.
          </p>
          <div className="mt-6">
            <AnalyticsCharts />
          </div>
